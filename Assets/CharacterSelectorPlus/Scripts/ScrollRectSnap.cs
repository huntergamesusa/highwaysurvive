...
public class ScrollRectSnap : MonoBehaviour {
	...
	void Start(){
		distance = new float[NewCharacterManager.myMat.Count];

//		distance = new float[prefab.Length];

		//instatiate the prefab

		for (int i = 0; i < NewCharacterManager.myMat.Count; i++) {
			prefab.Add(Instantiate(genericPrefab,center.transform.position,Camera.main.transform.rotation) as GameObject);
			...
			myMeshGO.GetComponent<SkinnedMeshRenderer> ().material = NewCharacterManager.myMat[i];
			foreach(KeyValuePair<Material,Mesh> keyValue in NewCharacterManager.myCharacters)
			{
				if (NewCharacterManager.myMat [i] == keyValue.Key) {
